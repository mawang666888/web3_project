...
//部署测试
//第一个账号先部署MyToken、再部署CrownFund  _token就是mytoken的合约地址
//需要对MyToken先approve ：spender是CrownFund众筹合约的地址
//账号1 (deployer):->launch
//账号2 ->pledge
//账号3 ->pledge
...
contract CrowdFund {
    ...
    struct Campaign {
        address creator;
        uint goal;
        uint pledged; //已经筹集到的资金
        uint32 startAt;
        uint32 endAt;
        bool claimed; //是否已经提款
    }

    IERC20 public immutable token; //筹集资金所需的token
    uint public count;
